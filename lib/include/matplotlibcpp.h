...
// Added BY ME
template<typename Numeric>
bool broken_barh(const std::vector<Numeric> &x, const std::vector<Numeric> &y, std::string fc = "tab:orange", const std::map<std::string, std::string> &keywords = { }) {
    ...
    PyDict_SetItemString(kwargs, "facecolors", PyUnicode_FromString(fc.c_str()));
    ...
    PyObject *res1 = PyObject_CallObject(detail::_interpreter::get().s_python_function_subplots, args);
    PyObject *ax = PyTuple_GetItem(res1,1);
    ...
    // PyObject *res = PyObject_Call(detail::_interpreter::get().s_python_function_broken_barh, plot_args, kwargs);
    // PyObject_CallFunction(ax, const_cast<char*>("broken_barh"),plot_args, kwargs);
    PyObject *broken_barh = PyObject_GetAttrString(ax, "broken_barh");
    PyObject *res = PyObject_Call(broken_barh, plot_args, kwargs);
